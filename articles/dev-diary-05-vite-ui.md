...
emoji: "🎨"
type: "tech"
topics: ["react", "vite", "claudecode", "css", "個人開発"]
published: false
---
...
一方のViteはESモジュールをネイティブに扱い、開発サーバー起動がほぼ一瞬です。
...
CRAとの違いで最初に気になったのは起動コマンドが `npm start` → `npm run dev` になったことくらいで、React自体の書き方は変わりません。
...
AIにUIを作ってもらうとき、「癒し系にして」「おしゃれにして」だと毎回違うものが出てきます。
特にコンポーネントをまたいで一貫したスタイルを維持するのが難しかった。

### 3-2. awesome-design-md の発想

GitHubに [awesome-design-md](https://github.com/akveo/awesome-design-md) というリポジトリがあります。
Notion・Linear・Figmaなど有名プロダクトのデザインシステムを、AIが読めるMarkdown形式でまとめたもの。

これをAIに渡すと「そのプロダクトのスタイルで実装して」が一発で通るようになります。

発想は単純で、**「いい感じ」の定義を先にMarkdownで書いておく**だけです。
