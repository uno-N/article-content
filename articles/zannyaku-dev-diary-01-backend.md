...
## 後日談：真犯人は「地理」だった（2026年7月追記）

この記事を下書きしてから2ヶ月後。別の作業で、自分のAPIが「どこで動いているか」を調べる機会がありました。

そこで判明した事実。

私のバックエンドは、**バージニア（米国東海岸・iad1）で動いていました**。データベース（Supabase）は東京。つまりログインのたびに、リクエストが太平洋を往復していたのです。

§6で「GoTrueに物理的に届いていない」と書きましたが、真相はおそらく「バージニアから東京のGoTrueへの長距離通信が、経路のどこかで死んでいた」。504の犯人は、コードでも設定でもなく、**地理**でした。

そしてもうひとつ。§5の試み④を読み返してください。

> リージョンを東京（nrt1）に変えようとした。無料プランでは使えなかった。

実はこれ、誤解でした。「nrt1」は Vercel には存在しないリージョン名で、東京は「**hnd1**」。正しいコードなら、無料プランでも普通に設定できました。

```json
// vercel.json に1行足すだけだった
{
  "regions": ["hnd1"]
}
```

3週間の格闘のなかで、真犯人にあと一歩まで迫っていたのに、リージョンコードひとつで取り逃していた——。

いまはAPIも東京で動き、データベースとの通信は国内で完結しています。ちなみにRPCによる迂回路は、GoTrueへの依存が減るという別のメリットもあったので、そのまま使い続けています。回り道も、無駄ではなかったようです。

エラーメッセージは色々教えてくれていたけれど、**地図までは見ていなかった**、という話でした。
